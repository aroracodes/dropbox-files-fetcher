...
// Route to fetch files from Dropbox and render the files.ejs view
            ...
            .then((response) => {
                const files = response.result.entries.map((file) => {
                    return {
                        name: file.name,
                        size: file.size,
                        modified: new Date(file.client_modified).toLocaleString(),
                        thumbnailUrl: file.thumbnail ? file.thumbnail : 'https://via.placeholder.com/150', // Placeholder image if thumbnail is not available
                    };
                });
